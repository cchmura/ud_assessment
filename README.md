...
%% Adapters
NFLKafka --> NFLIngest[NFL Ingest Adapter<br/>(Go Service)]
NBAPush --> NBAIngest[NBA Ingest Adapter<br/>(Go Service)]
MLBPubSub --> MLBIngest[MLB Ingest Adapter<br/>(Go Service)]

%% Internal Kafka - Raw
NFLIngest --> RawKafka[Internal Kafka Cluster<br/>Topics: nfl.raw, nba.raw, mlb.raw<br/>Partitions: 50/10/5<br/>Replication: 3x]
NBAIngest --> RawKafka
MLBIngest --> RawKafka

%% Event Normalizer
RawKafka --> Normalizer[Event Normalizer<br/>• Schema validation<br/>• Format standardization<br/>• Metadata enrichment<br/>• Deduplication]

%% Internal Kafka - Normalized
Normalizer --> NormKafka[Internal Kafka Cluster<br/>Topics: nfl.normalized, nba.normalized, mlb.normalized]

%% Odds Calculators
NormKafka --> NFLOdds[NFL Odds Calculator<br/>(Python/Rust)]
NormKafka --> NBAOdds[NBA Odds Calculator<br/>(Python/Rust)]
NormKafka --> MLBOdds[MLB Odds Calculator<br/>(Python/Rust)]

%% Odds Publisher
NFLOdds --> Publisher[Odds Publisher Service<br/>• Writes to:<br/>- External Kafka<br/>- ClickHouse<br/>- Redis]
NBAOdds --> Publisher
MLBOdds --> Publisher

%% Output Destinations
Publisher --> Redis[Redis Cache<br/>Cluster Mode - 3 Nodes]
Publisher --> ClickHouse[ClickHouse Analytics Platform<br/>ReplacingMergeTree + S3 Tiering]
