...
This infrastructure design outlines a scalable, resilient, and cost-conscious system for processing and distributing real-time sports odds updates across NFL, NBA, and MLB games. Given the stringent latency and uptime requirements—particularly during live games—we adopt a **cloud-native, event-driven microservices architecture** that prioritizes low-latency streaming, horizontal scalability, and robust observability.
...
We leverage **Infrastructure as Code (Terraform)**, automated CI/CD pipelines, and centralized observability tooling to ensure operational consistency, rapid deployment, and system-wide visibility.

---

## Key Decision Questions

### What is our primary architectural philosophy?

We adopt a **cloud-native, event-driven microservices architecture**:
...
### How do we balance cost, performance, and reliability?
...
We design for **burst tolerance** and **horizontal elasticity**:
...
- We use a **pure event-driven microservices approach** to enable real-time responsiveness, modular deployments, and system resilience.
...
We choose **Amazon Web Services (AWS)** as the primary cloud provider due to:
...
- We start with **single-cloud, multi-region** deployment for simplicity and redundancy.
...
#### Decision: VPC Design
- Each environment (dev, staging, prod) gets its own **isolated VPC**.
...
- If hybrid or partner cloud integrations are required, we use **VPN tunnels or AWS Direct Connect**.
...
While Kafka is our preferred ingestion backbone due to its performance, durability, and decoupling capabilities, **we cannot use Kafka for all integrations** without intermediate adapters. Some upstream feeds (e.g., Push Streams or third-party Pub/Sub services) do not natively support Kafka. Therefore:

- **Kafka is the unified internal messaging backbone**, but...
- **We adapt non-Kafka sources** using lightweight ingest gateways that convert Push or Pub/Sub protocols into Kafka messages.
...
We select **Amazon EKS (Elastic Kubernetes Service)** as our managed Kubernetes distribution due to:
...
- We choose **fully managed EKS** to reduce operational overhead and improve stability.
...
- We use **eksctl** or **Terraform** for cluster creation and version upgrades.
...
- We deploy **separate clusters per environment** (dev, staging, prod) for isolation and policy enforcement.
...
- We define CPU/memory **requests and limits** per container to ensure fair scheduling and avoid noisy neighbors.
...
- For stateful data like Redis or Postgres, we use **transaction boundaries** scoped to individual services.
...
- We use **Amazon RDS for PostgreSQL** in multi-AZ configuration with automatic failover for high availability.
...
- We use **ClickHouse** as the primary analytics engine due to its columnar storage, ultra-fast OLAP querying, and compatibility with real-time data ingestion.
- ClickHouse runs in the same EKS cluster or is managed via Altinity.Cloud for simplified ops.
...
- We use **Amazon MSK (Managed Streaming for Apache Kafka)** for internal event streaming to ensure scalability, high availability, and integration with AWS IAM and networking.
...
- We deploy the **Grafana/Loki/Prometheus/Tempo** stack for a unified observability plane, integrated with AWS services.
...
We adopt a hierarchy based on **AWS account → region → component**, allowing clean separation of environments, cross-account policies, and modular deployment.
...
| ClickHouse on EC2+S3   | Prod        | 3 nodes + storage    | $1,200               |
